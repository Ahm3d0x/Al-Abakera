-- ============================================================
-- Mind Race — Phase 5.2 Badge System Migration
-- ============================================================

-- 1. Helper function to award a badge to a user if not already earned
CREATE OR REPLACE FUNCTION award_badge_if_not_earned(p_user_id UUID, p_badge_key TEXT)
RETURNS VOID AS $$
DECLARE
  v_badge_id UUID;
BEGIN
  ...
  IF v_badge_id IS NOT NULL THEN
    INSERT INTO badges_earned (user_id, badge_id)
...
-- 2. Expand match ended trigger function to track categories, consecutive wins, captain wins, and evaluate match-specific badges
...
-- 3. Automatic badges checker trigger on profiles updates
CREATE OR REPLACE FUNCTION trigger_check_profile_badges()
RETURNS TRIGGER AS $$
DECLARE
  v_stats JSONB;
  v_played INTEGER;
...
BEGIN
  ...
  v_consecutive_wins := coalesce((v_stats->>'consecutiveWins')::integer, 0);
  v_captain_wins := coalesce((v_stats->>'captainWins')::integer, 0);
  v_science_correct := coalesce((v_stats->>'correct_Science')::integer, 0);
  v_history_correct := coalesce((v_stats->>'correct_History')::integer, 0);
  v_best_survival := coalesce((v_stats->>'bestSurvivalLevel')::integer, 0);
  v_daily_streak := coalesce((v_stats->>'dailyStreak')::integer, 0);

  -- 1. First Victory
  IF v_won >= 1 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'first_win');
  END IF;

  -- 2. Century (100 matches played)
  IF v_played >= 100 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'century');
  END IF;

  -- 3. Undefeated (50 consecutive wins)
  IF v_consecutive_wins >= 50 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'undefeated');
  END IF;

  -- 4. Team Leader (100 wins as captain)
  IF v_captain_wins >= 100 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'team_leader');
  END IF;

  -- 5. Scientist (1,000 science questions correct)
  IF v_science_correct >= 1000 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'scientist');
  END IF;

  -- 6. Historian (1,000 history questions correct)
  IF v_history_correct >= 1000 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'historian');
  END IF;

  -- 7. Survivor (Reach level 50 in Survival Mode)
  IF v_best_survival >= 50 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'survivor');
  END IF;

  -- 8. Daily Devotee (Complete 30 daily challenges in a row)
  IF v_daily_streak >= 30 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'daily_devotee');
  END IF;

  -- 9. Knowledge Titan
  IF NEW.rank_points >= 9000 THEN
    PERFORM award_badge_if_not_earned(NEW.id, 'knowledge_titan');
  END IF;
...
DROP TRIGGER IF EXISTS trg_check_profile_badges ON profiles;
CREATE TRIGGER trg_check_profile_badges
  AFTER INSERT OR UPDATE OF stats, rank_points ON profiles
  FOR EACH ROW
  EXECUTE FUNCTION trigger_check_profile_badges();
